...
スタッフの属性には、スケジュールの **リソース** フィールドからアクセスします。 既存のリソースを検索するか、 **作成** を選択して **簡易作成** ウィンドウで、新しいリソースとしてプロジェクト チーム メンバーを追加します。  タスク グリッド、ボード ビュー、ガントでリソース ピッカーを使ってリソースを検索すると、既存のプロジェクト チーム メンバーか、アクティブな予約可能リソースが検索結果として表示されます。

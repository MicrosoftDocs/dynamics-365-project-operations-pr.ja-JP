...
description: この記事は、プロジェクト スケジュール アプリケーション プログラミング インターフェイス (API) を使用するサンプル フローを提供します。
author: ruhercul
ms.date: 01/26/2022
ms.topic: article
...
以下は、この記事のサンプル フローに記載されているステップの完全なリストです。
...
この記事は、Dataverse プラットフォーム、クラウド フロー、プロジェクト スケジュール アプリケーション プログラミング インターフェイス (API) の基本的な知識がある事を推定しています。 詳細については、この記事の後にある [参照](#references) セクションをご覧ください。
...
4. **このフローをトリガーする方法を選択** リストで、**Power Apps** を選択します。 Power Apps トリガーを作成する時、ロジックは作成者としてのあなた次第です。 この記事では、テストのために入力パラメーターを空白のままにします。
5. **作成** を選択します。

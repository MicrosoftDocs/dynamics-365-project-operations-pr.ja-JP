...
![1 つのリソースに割り当てられた複数のロール。](media/upgrade-multiple-roles-02.png)
...
![既定ロールに対するコスト見積り。](media/resource-assignment-cost-estimate-06.png)
![既定ロールに対する営業見積り。](media/resource-assignment-sales-estimate-07.png)
...
たとえば、「プロジェクト Z」 プロジェクトでは、契約組織単位は、「Contoso US」 となります。 プロジェクト プランでは、実装段階内のテスト タスクは、技術コンサルタントのロールに割り当てられ、割り当てられた組織単位は、Contoso India です。

![実装段階の組織割り当て。](media/org-unit-assignment-09.png)

実装段階の後、統合テスト タスクは、技術コンサルタントのロールに割り当てられますが、組織は、「Contoso US」 に設定されます。  

![統合テスト タスク組織の割り当て。](media/org-unit-generate-team-10.png)

プロジェクトのチームが生成されると、2 名の汎用チーム メンバーが、タスクで異なる組織単位に対して作成されます。 技術コンサルタント 1 は、Contoso India に割り当てられ、技術コンサルタント 2 は、Contoso US を有します。  
...
![Project Service Automation バージョン 2 と バージョン 1 の行タスク。](media/line-tasks-12.png)
...
契約プロジェクトの組織単位と異なる組織単位、およびまだ生成されていないチームにロールを割り当てるタスクの場合は、アップグレードによってロールに対する汎用チーム メンバーを作成できますが、チーム メンバーの組織単位の場合はプロジェクトの契約単位を使用します。 「プロジェクト Z」 の例を再び見てみると、契約組織単位 「Contoso US」、および実装段階のプロジェクト プランのテスト タスクは、「Contoso India」 に割り当てられた組織単位で技術コンサルタントのロールが割り当てられています。 実装段階後に完了した統合テスト タスクは、ロールの技術コンサルタントに割り当てられます。 組織単位は 「Contoso US」 で、チームは生成されていません。 アップグレードによって、1 人の汎用チーム メンバーが作成され、3 つのすべてのタスクに対して割り当て時間を持つ技術コンサルタント、およびプロジェクトの契約組織単位である Contoso US の組織単位が作成されます。   
